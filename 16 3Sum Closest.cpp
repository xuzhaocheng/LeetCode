// 16 3Sum Closest.cpp : 定义控制台应用程序的入口点。
//

#include "stdafx.h"

#include <algorithm>
#include <vector>
#include <iostream>

using namespace std;

/*
class Solution {
public:
    int threeSumClosest(vector<int> &num, int target) {
		std::sort(num.begin(), num.end());      
		int ret = INT_MAX;
		for (int i = 0; i < num.size(); i++) {
			if (abs(num[i] - target) > abs(target - ret) && num[i] - target > 0)	break;
			for (int j = i + 1; j < num.size(); j++) {
				int twoSum = num[i] + num[j];
				if (abs(twoSum - target)> abs(target - ret) && twoSum - target > 0)	break;
				for (int z = j + 1; z < num.size(); z++) {
					if (abs(twoSum + num[z] - target) > abs(target - ret) && twoSum + num[z] - target > 0) break;
					else if (abs(twoSum + num[z] - target) < abs(target - ret))
						ret = twoSum + num[z];
				}
			}
		}
		return ret;
    }
};*/


// 排序后二分搜索
/*
class Solution {
public:
    int threeSumClosest(vector<int> &num, int target) {
		std::sort(num.begin(), num.end());
		
		int closestSum = num[0] + num[1] + num[2];

		for (int i = 0; i < num.size(); i++) {
			for (int j = i + 1; j < num.size(); j++) {
				int twoSum = num[i] + num[j];
				int left = j + 1, right = num.size() - 1;
				int mid;
				while ( left <= right) {
					mid = (left + right) >> 1;
					if (twoSum + num[mid] - target < 0) {
						if (target - (twoSum + num[mid]) < abs(target - closestSum))
							closestSum = twoSum + num[mid];
						left = mid + 1;
					} else if (twoSum + num[mid] - target > 0) {
						if (twoSum + num[mid] - target < abs(target - closestSum))
							closestSum = twoSum + num[mid];
						right = mid - 1;
					} else 
						return twoSum + num[mid];
				}
			}
		}

		return closestSum;
    }
};
*/

class Solution {
public:
    int threeSumClosest(vector<int> &num, int target) {
		std::sort(num.begin(), num.end());
		
		int closestSum = num[0] + num[1] + num[2];

		int sum;
		for (int i = 0; i < num.size(); i++) {
			int j = i + 1, k = num.size() - 1;
			while( j < k ) {
				sum = num[i] + num[j] + num[k];
				if (abs(sum - target) < abs(closestSum - target)) {
					closestSum = sum;
					if (closestSum == target)	return target;
				}

				if (sum - target < 0)	j++;
				else k--;
			}
		}

		return closestSum;
    }
};


int _tmain(int argc, _TCHAR* argv[])
{
	Solution s;

	int n[] = {43,75,-90,47,-49,72,17,-31,-68,-22,-21,-30,65,88,-75,23,97,-61,53,87,-3,33,20,51,-79,43,80,-9,34,-89,-7,93,43,55,-94,29,-32,-49,25,72,-6,35,53,63,6,-62,-96,-83,-73,66,-11,96,-90,-27,78,-51,79,35,-63,85,-82,-15,100,-82,1,-4,-41,-21,11,12,12,72,-82,-22,37,47,-18,61,60,55,22,-6,26,-60,-42,-92,68,45,-1,-26,5,-56,-1,73,92,-55,-20,-43,-56,-15,7,52,35,-90,63,41,-55,-58,46,-84,-92,17,-66,-23,96,-19,-44,77,67,-47,-48,99,51,-25,19,0,-13,-88,-10,-67,14,7,89,-69,-83,86,-70,-66,-38,-50,66,0,-67,-91,-65,83,42,70,-6,52,-21,-86,-87,-44,8,49,-76,86,-3,87,-32,81,-58,37,-55,19,-26,66,-89,-70,-69,37,0,19,-65,38,7,3,1,-96,96,-65,-52,66,5,-3,-87,-16,-96,57,-74,91,46,-79,0,-69,55,49,-96,80,83,73,56,22,58,-44,-40,-45,95,99,-97,-22,-33,-92,-51,62,20,70,90};
	int target = 284;
	vector<int> num;

	for (int i = 0; i < sizeof(n) / sizeof(n[0]); i++) 
		num.push_back(n[i]);

	cout << s.threeSumClosest(num, target) << endl;
	return 0;
}

